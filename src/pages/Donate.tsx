...
export default function Donate() {
  ...
  const initKcbCheckout = async () => {
    ...
    try {
      ...
      // Guard against non-JSON responses (e.g. 405 HTML page if the
      // serverless function route is not live yet) — never let res.json()
      // throw "Unexpected end of JSON input" at the donor.
      let data: any = null
      try {
        data = await res.json()
      } catch {
      ...
      if (!res.ok) {
        if (res.status === 503 && data?.error?.includes('not yet configured')) {
          ...
          setKcbError(data.error)
        } else {
          setKcbError(`Card payment service is temporarily unavailable (HTTP ${res.status}). Please try again in a moment or use M-Pesa below.`)
        }
        setKcbLoading(false)
        return
      }
      ...
      if (!data?.sessionJWT) {
        setKcbError('Could not start the card payment session. Please try again.')
        setKcbLoading(false)
        return
      }
      ...
      script.onload = async () => {
        try {
          ...
          if (!verifyRes.ok || !verifyData) {
            throw new Error(verifyData?.error || `Payment verification service error (HTTP ${verifyRes.status})`)
          }
